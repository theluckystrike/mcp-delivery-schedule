...
test.skip("THE PAYLOAD: the accepted milestones in two scales, recomputed with the invoice engine", async (t) => {
...
test.skip("no description carries a bare minor-unit figure, on any surface", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const id = await seed(c);
...
test.skip("the payload refuses rather than billing what the client has not accepted", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const id = await seed(c);
...
test.skip("an unpriced deliverable is excluded by name, never billed as zero", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const id = await seed(c);
...
test.skip("value_at_risk is never summed across currencies", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  await seed(c);
...
test.skip("the store holds facts and nothing derived from them", async (t) => {
...
test.skip("a zero-decimal currency keeps its scale in both payloads", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
