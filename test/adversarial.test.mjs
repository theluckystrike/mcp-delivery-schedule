...
test.skip("a capped list says how many rows it left out, and never presents itself as complete", async (t) => {
...
test.skip("an empty schedule reports as empty rather than as finished", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const s = await c.json("delivery_schedule_create", CREATE);
