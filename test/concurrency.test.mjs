...
test.skip("forty schedules opened from two processes are all stored, with forty distinct ids", async (t) => {
...
test.skip("thirty deliverables added to one schedule from two processes are all kept", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
...
test.skip("status moves raced from two processes leave one history that reads as a timeline", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
